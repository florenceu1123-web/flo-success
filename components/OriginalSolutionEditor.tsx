...
/**
 * 오답 메모지 — 원본 문제별 풀이·정답 사진 + 오답 메모 + 생성문제 보관함.
 *
 * ★ AnalysisPanel 안에 있던 것을 파일로 분리했다. **문제 생성 화면과 랜덤문제 풀기가
 *   같은 메모지를 공유**해야 해서 두 곳에서 import 할 수 있어야 한다(사용자 지정).
 * ★ 저장 단위는 imageKey — 이미지 내용의 해시(lib/imageKey.ts)라, 같은 사진이면
 *   어느 화면에서 열든 같은 메모가 뜬다.
 */
 ...
 */
export function OriginalSolutionEditor({ imageKey }: { imageKey: string | null }) {
