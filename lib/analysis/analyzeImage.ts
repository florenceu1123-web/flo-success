...
- topic·interpretation·relatedConcepts에 ★어떤 물리 법칙·공식인지★ 반드시 보존하라. 분류 키워드 예:
  ...
  · ★무한 면전하 + 무한 **직선** 선전하 → 합성 전계 **벡터**로 두 밀도 역산★: ★x=k 평면(yz면과 나란)의 면전하 C₁★와 ★(x=0, z=m)에서 y축과 나란한 **무한 직선** 선전하 C₂★가 주어지고, ★축 밖의 점 P(a,b,c)★에서 합성 전계가 ★벡터(예: E = −162π a_x − 36π a_z)로 주어질 때 C₁·C₂를 **각각** 구하는★ 유형. → ★원형 루프/링 아님(반지름 없음)★, 크기 비 조건도 아님(합성 전계 벡터가 given), E=0 조건도 아님. 반드시 보존: "무한 평면 x=k"·"무한선(y축과 나란)"·점 P의 세 좌표·합성 전계 벡터의 두 성분·"C₁과 C₂를 각각"·ε₀=(1/36π)×10⁻⁹·해석 절차 3단계. topicKey=gauss_law. topic을 "원형 루프"·"전계의 크기 비"로 요약 금지.
  ...
  · ★원점에서 꺾인 반무한 직선 도선(축을 따라 흐르는 선전류) 합성 자계 → 전류 I★: 선전류가 ★무한히 먼 곳에서 한 좌표축(예: y축)을 따라 원점 O까지 흐른 뒤, 원점에서 다른 좌표축(예: x축)을 따라 무한히 먼 곳으로★ 나가는 ★하나의 꺾인 도선★. 점 P(a,b,0)에서 "y축 전류에 의한 H₁"·"x축 전류에 의한 H₂"의 합성 H₃=(값/π)a_z가 되는 I를 구한다. → ★원형 루프 아님(루프·반지름 없음)★, 평행한 두 도선도 아님(도선은 하나이고 원점에서 꺾인다). 반드시 보존: "무한히 먼 곳"·"○축을 따라"·"원점 O"·각 구간의 전류 방향(−a_y·+a_x)·점 P 좌표·"합성 자계 H₃"·해석 절차 3단계. topicKey=magnetostatics. topic을 "원형 루프"·"두 평행 도선"으로 요약 금지.
  ...
  · ★두 무한 직선 도선(전류 반대 방향) 합성 자계 → 위치 a 도출 → 단위 길이당 힘★: ★z축과 나란한 무한 직선 도선이 2개★(도선 A: x=0·y=y_A, 전류 +a_z 방향 / 도선 B: x=0·y=a, 전류 −a_z 방향)이고, 점 O(0,0,0)과 점 P(0,y_P,0)에서의 ★합성 자계 H_O·H_P를 a가 포함된 식★으로 구한 뒤, [2]★크기 비 |H_O|:|H_P| = m:n★이 되는 a를 구하고, [3]★도선 B에 작용하는 단위 길이당 힘 F [N/m]★를 구하는 유형. → ★단일 직선 도선 자기장(B=μ₀I/2πr) 아님(도선이 2개다), 면전류+선전류 합성 자계도 아니고(무한 평면 없음), 원형 루프도 아님★. 반드시 보존: "무한 도선 A·B 2개"·각 도선의 위치(x=0, y=y_A / y=a)·전류 세기와 ★방향(+a_z / −a_z, 서로 반대)★·점 O와 P의 좌표·"합성 자계"·"자계의 크기 비 m:n"·"a>y_P"·"단위 길이당 힘"·"진공 투자율 μ₀와 π는 그대로"·해석 절차 3단계. topicKey=magnetostatics. topic을 "직선 도선 자기장"(단일)·"면전류/선전류"·"원형 루프"로 요약 금지.
...
- ★★ **제어 흐름을 바꾸는 위치는 글자 그대로 보존**하라 — 특히 switch의 \`break\`가 **어느 라벨에 있고 어느 라벨에 없는지**.
  break가 없는 case는 다음 case/default로 이어지며(fall-through), 그것이 정답을 가르는 핵심인 경우가 많다.
  "각 case에서 break로 빠져나온다"처럼 **관용적으로 뭉뚱그려 적지 마라**. 배열을 부분 초기화해 뒤쪽이 0인 것도 그대로 적어라.
...
/**
 * ★ 과목별 프롬프트 슬라이싱 (2026-08-02) — 다른 과목 전용 archetype 규칙 블록을 제거한다.
 *
 * 왜: 회로 4과목(전자·회로이론·디지털·복합형)에 **하나의 ~25k 토큰 프롬프트**를 통째로 보내고 있어
 *   OpenAI **TPM 한도(30k/분)에 상시 밀착**해 있었다. 실측으로 규칙 몇 줄만 추가해도 429가 났다
 *   (`Requested 30322` / `30109` / `30013`). 디지털 문제에 OPAMP·테브난·페이저 규칙을,
 *   아날로그 문제에 카르노맵·플립플롭 규칙을 보낼 이유가 없다.
 *
 * 방식: 프롬프트는 줄 맨 앞의 `【헤더】`로 블록이 나뉜다(전부 46개, 확인됨). **헤더 문자열로만**
 *   판별해 해당 없는 블록을 뺀다 — 본문을 건드리지 않아 회귀 위험이 낮고 감사(audit)가 쉽다.
 *   · 복합형(mixed_signal)은 아날로그+디지털을 모두 쓰므로 **전체 유지**.
 *   · 목록에 없는 블록은 **항상 유지**(공유 규칙: 스키마·출력 규칙·개념형 판별 등).
 */
const DIGITAL_ONLY_HEADERS = [
  "digital_logic",                 // 헤더에 과목명이 박힌 블록 전부
  "플립플롭 종류는 상태표",          // D vs J-K 판별
  "JK 상태 여기표",                 // 2025 전기 A-8
];
const ANALOG_ONLY_HEADERS = [
  "annotation 추출", "topologySignature 추출", "betweenNodes",
  "few-shot — supermesh", "few-shot — 6번 horizontal V", "few-shot — 2-OPAMP",
  "few-shot — 임용 10번", "few-shot — 4-mesh",
  "AC 입력 회로", "RLC 공진", "AC 휘트스톤", "2전원 페이저", "DC 휘트스톤", "AC 역률보정",
  "어드미턴스 공진", "종속전류원", "SPDT 스위치", "스위치 RL", "2전원 SPDT",
  "이상 인덕터/커패시터", "단일 AC원", "종속전원 + 교류 테브난", "2개 전압원 병렬가지",
  "electronics OPAMP", "electronics 2-OPAMP",
  "circuit_theory AC", "circuit_theory 직류+교류", "circuit_theory Thevenin", "circuit_theory Multi-step",
  "semantic node = role-based", "dangling node 금지",
  "DC 중첩의 원리",
];

function sliceRulesBySubject(prompt: string, subject: SubjectKey): string {
  if (subject === "mixed_signal") return prompt;         // 복합형은 양쪽 규칙이 모두 필요
  const drop = subject === "digital_logic" ? ANALOG_ONLY_HEADERS : DIGITAL_ONLY_HEADERS;
  const parts = prompt.split(/\n(?=【)/);
  const kept = parts.filter((part) => {
    const header = /^【([^】]*)】/.exec(part)?.[1];
    if (!header) return true;                            // 헤더 없는 선두 조각(공통 서문)
    return !drop.some((h) => header.includes(h));
  });
  return kept.join("\n");
}
...
function buildPrompt(subject: SubjectKey): string {
  ...
  return sliceRulesBySubject(buildCircuitPrompt(subject), subject);
}

function buildCircuitPrompt(subject: SubjectKey): string {
...
【종속전원 + 교류 테브난 등가(단락전류법) + 켤레 최대전력 (임용 6번 회로이론, ac_thevenin_dependent)】
- 독립 전원 + ★종속 전원(다이아몬드, 값 k·I_x/k·V_x)★ 이 있는 페이저 회로에서 단자 A–B 테브난 등가와 부하 Z_L=R+jX 최대 평균 전력을 구하는 형식이면:
  interpretation에 "종속 전원"·"테브난 등가 V_AB"·★"A–B 단락 전류 I_AB로 Z_AB"★·"Z_L=R+jX 켤레 정합"·"최대 평균 전력"을 명시.
  inventory는 독립 전원 1 + ★종속 전원 1(type CCVS/CCCS/VCVS/VCCS, value는 제어식 그대로)★ + 리액티브(jX·−jX). 다이아몬드를 일반 V·I로 적지 마라.
  ★금지★: "스위치·t=0·과도응답·시정수"로 요약(이 회로엔 스위치가 없다 — 직류 스위치 RL 과도로 변질), 순저항 R_L로 적기, 공진·브리지로 요약. topicKey=dependent_source.
...
【★ digital_logic 상태도에 ★입력 X★가 있는 경우 — 절대 추출 규칙 (tff_state_design_input 라우팅 핵심, 임용 12번)】

다음 중 ★ 하나라도 ★ 보이면 이 형식이다 (아래 "자율 상태도"와 반드시 구분):
  (1) 상태도의 화살표 옆에 ★ 0 / 1 숫자 라벨 ★ 이 붙어 있다 (같은 상태에서 두 갈래로 갈라진다).
  (2) 상태표에 ★ "입력" 열(X) ★ 이 있어 행이 ★ 8개 ★ 다 (4개가 아니다).
  (3) 카르노맵의 축이 ★ Q_AQ_B(행) × X(열) ★ — 즉 3변수다.

이 경우 ★ 반드시 ★:
(A) topic·interpretation에 ★ "입력 X" ★ 를 명시하라.
    예: "상태 변수 Q_A, Q_B와 ★입력 X★를 갖는 상태도로부터 상태표·카르노맵을 작성해 플립플롭 입력을 설계"
(B) relatedConcepts에 "입력 X"를 포함하라.
(C) ★ 절대 금지 ★: 입력 X를 빠뜨리고 "상태도를 바탕으로 상태표를 작성한다"로만 요약하는 것.
    → 입력 없는 자율 상태도 유형으로 변질되어 ★ 원본에 있는 입력 X가 사라진 ★ 문제가 생성된다 (실측 3회).
(D) 상태도의 각 화살표에 붙은 입력값(0/1)과 다음 상태를 fillInTheBlanks·interpretation에 최대한 보존하라.

【★ digital_logic 자율(입력 없는) 상태도 → 플립플롭 + 게이트 설계 — 절대 추출 규칙 (dff_state_design 라우팅 핵심)】

시각 단서 2개 이상이면 이 형식이다:
  (1) (가) 상태도 — 원 노드 4개(00·01·10·11) + 화살표.
      ★★ 화살표에 0/1 라벨이 있거나 상태표에 ★입력 X 열(8행)★이 있으면 → interpretation에 ★"입력 X"★ 반드시 명시.
      라벨이 전혀 없으면 자율 순환(autonomous) — 아래 (D) 적용.
  (2) (나) 상태표 — [현재상태 Q_A·Q_B] | (입력 X) | [다음상태 ㉠~㉣] | [플립플롭 입력 D_A·D_B].
  (3) (다) 구현 회로 — 플립플롭 2개 + 게이트 빈칸(㉮·㉯ 또는 ㉡) + Q 피드백 + 공통 CLK.
  (4) [설계 절차] [단계1] 다음상태 [단계2] FF 입력 불함수 [단계3] 게이트 구성.

★ FF 종류 판별: 입력 열 D_A·D_B → D. J_A·K_A → J-K. T_A·T_B → T-FF. 열이 안 보이면 D로 간주.

(A) topicKey를 "fsm"으로 적지 마라 — generic Mealy FSM(입력·출력 날조)으로 라우팅된다. 순서논리 설계로.
(B) topic·interpretation에 ★"D(또는 T) 플립플롭"+"상태도"+"게이트 설계"★ 명시. "상태 전이도 분석"처럼 뭉뚱그리지 마라.
(C) relatedConcepts 4개 이상: 플립플롭·상태도·상태표·게이트·순서논리회로·다음 상태·K-map·SOP 중.
(D) 자율 순환(화살표 라벨 없음)이면 ★입력 X·출력 Z를 만들어내지 마라★. 반대로 라벨이 있으면 입력 X를 빠뜨리지 마라.
(E) 빈칸 마커(㉠㉡㉢㉣·㉮㉯)를 그대로 transcribe해 fillInTheBlanks에 넣어라.
★ 양보: 화살표에 x/y(출력) 라벨 → fsm. "J-K" 명시 → J-K. SR+MUX → SR-MUX 순차설계.
...
/** 감사·스모크 전용 — 과목별 최종 프롬프트를 그대로 반환한다(토큰 예산 회귀 검사용). */
export function __buildPromptForAudit(subject: SubjectKey): string {
