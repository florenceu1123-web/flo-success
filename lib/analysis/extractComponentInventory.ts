...
【★★ 능동소자(OPAMP·BJT·MOSFET) 추출 — 최우선 절대 규칙 ★★】
★ 삼각형(▷, 꼭짓점이 출력) 심볼은 라벨(U1 등)이 없어도 ★ 무조건 OPAMP ★ 로 추출하라.
★ topic/본문에 "연산 증폭기"·"OP-amp"·"증폭기"가 있으면 회로에 OPAMP가 ★ 반드시 ★ 있다 —
  삼각형을 빠짐없이 세어 그 개수만큼 OPAMP 항목을 componentInventory에 넣어라.
  (실패 사례: 라벨 없는 삼각형 2개를 건너뛰고 저항만 추출 → 회로가 단순 저항망으로 오인되어
   풀이 전체가 엉뚱한 답을 냄. ★ 저항만 있고 OPAMP 0개면 거의 항상 추출 실패다 ★.)
- OPAMP가 K개 보이면 inventory에 "OPAMP" ★ K개 ★ ({ "type":"OPAMP", "id":"U1" } 등).
- 각 OPAMP의 +/− 입력·출력이 어느 노드/소자에 연결되는지 value나 id에 가능한 한 메모.
- BJT(Q, npn/pnp)·MOSFET(M)·다이오드(D)·종속전원(◇)도 동일하게 빠짐없이.

【★ 완전성(빠짐없이) — 절대 규칙】
추출을 마치기 전, 회로의 ★ 모든 가지(branch) ★ 를 훑으며 셀프 체크하라:
  · ★ 삼각형 OPAMP·트랜지스터 등 능동소자를 먼저 세었는가 ★ (위 규칙)
