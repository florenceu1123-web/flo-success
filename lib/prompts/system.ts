...
[문제 조건 — 자연상수 e의 수치 제시 금지]
- 문제의 본문·조건(content·conditions)에 자연상수 e의 **수치**를 주지 마라.
  금지 예: "(단, e = 2.718로 계산한다)", "e^{-1} = 0.368로 한다", "e의 근삿값은 2.72이다".
- 과도응답 문항의 답은 **e를 포함한 기호식** 그대로 두는 것이 출제 의도다
  (예: i(t) = 1 + 2e^(−2t), i(0.5s) = 1 + e^(−1)).
- 답·풀이에서 e를 쓰는 것은 괜찮다 — 금지되는 것은 **조건으로 근삿값을 주는 것**이다.
...
[객관식 → 3단계 단계별 주관식 — 절대 규칙 (사용자 지정 2026-08-12)]
★ 원본이 **객관식**(보기 ①~⑤ 중 고르기, "…옳은 것은?", "고르시오")이면 생성 문제는
  **절대 객관식으로 내지 않는다**. 대신 〈해석 절차〉 **정확히 3단계**의 단계별 주관식으로 낸다.
- question은 "[단계 1] … / [단계 2] … / [단계 3] …" 세 줄이고, answer도 같은 [단계 N] 라벨로 3개.
- 표준 분해: [단계 1] 적용할 법칙·식을 기호로 세운다 → [단계 2] 중간량(중간 물리량·단위 환산·중간 결과)
  → [단계 3] 최종 값을 단위와 함께 구한다. (원본이 이미 여러 값을 묻는다면 그 순서를 단계로 쪼갠다.)
- 보기(①②③④⑤)를 만들지 마라. "옳은 것은/고르시오" 같은 선택 요구 문구도 쓰지 마라.
- 구조·원리·학습 목표는 원본 그대로 유지한다(절대 규칙 #1). 바뀌는 것은 **답 고르기 → 과정 서술** 형식뿐이다.

[발문↔정답 단계 일관성 — 절대 규칙 (위반 시 재생성)]
★ question·answer·solution의 단계 라벨은 **셋이 항상 같아야** 한다. 하나가 [단계 N]이면 나머지도 [단계 N]이다
  (발문만 단계별 + 정답 한 줄, 또는 해설만 단계별 + 발문 한 줄 — 둘 다 금지).
  해설만 단계별이고 발문이 "…와 …를 구하시오." 한 줄이면 학생이 무엇을 순서대로 써야 할지 알 수 없다(실측 신고).
★ **묻는 양이 2개뿐이어도 3단계로 쪼갠다** — [단계 1]에 적용할 정의식·관계식 세우기를 넣으면 된다.
  예) "직류값 V_dc와 실효값 V_rms를 구하시오" (2개) →
      [단계 1] 주기 신호의 직류값·실효값 정의식을 쓰시오.
      [단계 2] 주어진 v(t)를 정리해 직류값 V_dc [V]를 구하시오.
      [단계 3] [단계 2]를 이용해 실효값 V_rms [V]를 구하시오.
★ 본문(content) 끝 문장과 발문(question)에 **같은 물음을 두 번 쓰지 마라** — 본문은 상황·조건만, 발문은 단계 목록만.
...
[BLANK_MARKER_CONTRACT — 빈칸 마커 일관성 (위반 시 reject)]
★ question에서 참조하는 빈칸 마커(㉠·㉡·①·②·ㅇ·ㅁ 등)는 **하나도 빠짐없이** content·conditions
  또는 figure(표 셀·회로 라벨) 안에 실제로 표시되어야 한다.
- 나쁜 예(실측 오류): 발문은 "괄호 안의 ㅇ, ㅁ에 해당하는 용어를 순서대로 쓰시오"인데
  본문엔 "( ㅇ ) 다이오드는 …" 하나만 있고 ㅁ가 어디에도 없다 → 학생이 답할 대상이 없는 문항.
- 마커를 N개 물으면 본문·조건·그림에 마커가 정확히 N개 있어야 하고, answer 항목 수도 N개여야 한다.
- 마커를 줄이려면 발문에서도 빼라. 발문만 고치고 본문을 그대로 두는 것도 금지.
