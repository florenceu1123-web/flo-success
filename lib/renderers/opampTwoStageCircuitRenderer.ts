...
export function renderOpampTwoStage(d: D): string {
  ...
  // ── V_i 전원 (좌측 세로) ──
  s.push(acSource(VI_X, MID_Y - 28, GND_Y));
  t.push(text(VI_X - 28, (MID_Y - 28 + GND_Y) / 2, d.viLabel ?? "v_i", { anchor: "end", size: 13, weight: 700, fill: ACCENT }));
  // V_i top → U₁ (+)입력 (비반전)
  w.push(line(VI_X, MID_Y - 28, VI_X, p1.plus.y));
  ...
  // ── V_P → Rin2 → U₂ (−)입력 (반전) ──
  const RIN2_L = VP_X + 24;
  w.push(line(VP_X, p1.out.y, RIN2_L, p1.out.y));
  hResistor(s, RIN2_L, RIN2_L + 56, p2.minus.y);
  w.push(line(RIN2_L + 56, p2.minus.y, p2.minus.x, p2.minus.y));
  t.push(text(RIN2_L + 28, p2.minus.y - 8, d.rin2Label ?? "Rin2", { size: 11, weight: 600 }));
