...
import { extractSequentialLogic, convertOneDffToTff, enforceSecondFfInvertedClock } from "@/lib/generation/digital/extractSequentialLogic";
...
async function writeText(args: { states: Record<string, string>; outputs: string[]; ffKinds?: string[] }): Promise<Txt> {
  const openai = getOpenAI();
  const { states, outputs } = args;
  const qLabel = outputs.join("");
  const answer = POINTS.map((p) => `${p.sym}: ${qLabel}=${states[p.sym]}`).join(", ");
  // 회로의 FF 종류 표기 — 혼합(D-FF·T-FF)이면 본문이 정확히 명시하도록.
  const ffKinds = args.ffKinds && args.ffKinds.length ? args.ffKinds : ["D"];
  const ffPhrase = ffKinds.length > 1
    ? `${ffKinds.map((k) => `${k} 플립플롭`).join("과 ")}을 함께 사용한`
    : "D 플립플롭을 이용한";
  const userPrompt = [
    `[출력 비트] ${qLabel} (MSB 먼저)`,
    `[플립플롭 구성] ${ffKinds.map((k) => `${k}-FF`).join(" + ")} (${ffPhrase} 회로)`,
    `[각 지점 상태 — 변경 금지] ${answer}`,
    `[확정 정답] ${answer}`,
    ``,
    `위 순서논리 문제(content/question/solution) 작성. content는 "${ffPhrase} 회로"임을 명시.`,
    `T 플립플롭이 포함되면 풀이는 Q_next = Q ⊕ T 규칙을, D 플립플롭은 Q_next = D 규칙을 사용. answer는 [확정 정답] 그대로.`,
  ].join("\n");
  ...
  return {
    content: parsed.content ?? `그림 (가)는 ${ffPhrase} 회로, (나)는 입력 파형이다. <해석 절차>에 따라 구하시오.`,
...
export async function runSequentialGenericPipeline(args: {
  ...
  return generateInParallel(count, async (i, seed) => {
    ...
    // 원본 클럭 구조(유사·변형 공통): 첫 FF는 클럭 직결, 두 번째 FF는 NOT 게이트 거친 반전 클럭(CLK').
    let diagram = enforceSecondFfInvertedClock(ex.diagram, order);
    // 기출변형유형: 두 D-FF 중 하나를 T 플립플롭으로 등가 변환 (D-FF 1 + T-FF 1).
    //   상태 시퀀스·정답은 동일하게 유지(T = D ⊕ Q) — 소자 종류만 변형.
    if (mode === "exam_variant") diagram = convertOneDffToTff(diagram, seed);
    const spec = seqSpecFromLogicNetwork(diagram);
    // 회로에 실제 존재하는 FF 종류 (라벨·본문 표기용)
    const ffKinds = Array.from(
      new Set(diagram.gates
        .filter((g) => g.type === "DFF" || g.type === "TFF" || g.type === "JKFF")
        .map((g) => (g.type === "DFF" ? "D" : g.type === "TFF" ? "T" : "JK"))),
    );
    const ffLabel = ffKinds.length > 1
      ? `(가) 플립플롭 회로 (${ffKinds.map((k) => `${k}-FF`).join("·")})`
      : "(가) D 플립플롭 회로";
    ...
    const text = await writeText({ states, outputs: order, ffKinds });
