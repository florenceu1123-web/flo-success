...
export type Imyong10Component =
  | { id: string; type: "V"; value?: string; p: string; n: string }
  | { id: string; type: "R" | "R_VAR" | "I"; value?: string; a: string; b: string };
...
export function validateImyong10Topology(c: Imyong10Netlist): string[] {
  ...
  const vsrc = c.components.find((x) => x.type === "V") as
    | (Extract<Imyong10Component, { type: "V" }>)
    | undefined;
  if (!vsrc) {
  ...
  const rvar = c.components.find((x) => x.type === "R_VAR") as
    | (Extract<Imyong10Component, { type: "R_VAR" }>)
    | undefined;
  if (!rvar) {
