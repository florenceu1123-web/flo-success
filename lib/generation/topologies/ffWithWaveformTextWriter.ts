...
export async function writeFfWithWaveformText(args: {
  ...
  const { xExpression, yExpression } = generation;

  const userPrompt = `다음 정보로 임용 8번 형식의 "D-플립플롭 + 게이트 응용 회로 + 파형" 문제를 작성하세요.
회로·파형 데이터는 코드가 이미 결정 — 너는 문제 문장과 풀이만 작성.

[원본 형식 — 그림 (가), (나)]
그림 (가): 게이트들과 비동기 RESET 입력을 갖는 D-플립플롭으로 구성된 응용 회로.
  · 외부 입력: A, B, C  (3개만 — CLK는 외부 입력이 아니다)
  · 내부 신호: X = SOP(A, B, C) → D-플립플롭의 CLK 핀(▷)에 연결 (게이트 출력이 곧 클럭)
                Y = SOP(A, B, C) → D-플립플롭의 R 핀 (비동기 RESET)
  · D 핀 ← Q'(자기 피드백) — 매 클럭 상승 에지에서 Q 토글.
  · 출력: Q. Q 초기값 = 0.
...
[출력 JSON]
{
  "content":    "그림 (가)는 게이트들과 비동기 리셋(RESET) 입력을 갖는 D-플립플롭으로 구성된 응용 회로이다. 입력 신호 A, B, C가 그림 (나)와 같이 입력될 때, 제시된 <해석 절차>에 따라 각 단계별 풀이 과정과 결과를 서술하시오. (단, 그림 (가)에서 버퍼의 전파 지연 시간(propagation delay time)을 제외하고 모든 소자의 특성은 이상적이다. D-플립플롭 입력의 변화는 그림 (나)의 버퍼 전파 지연 시간 tp만큼 지연되어 감지된다.)",
  "conditions": ["외부 입력은 A, B, C 3개", "내부 신호 X가 D-플립플롭의 CLK 입력으로 동작", "내부 신호 Y가 D-플립플롭의 비동기 RESET으로 동작", "D 핀에는 Q' 피드백 (자기 토글)", "Q 초기값 = 0", "버퍼 전파 지연 tp 제외 모든 소자 이상적"],
  "question":   "[단계 1] 입력 A, B, C를 이용하여 출력 X에 대한 최소화된 불 함수(Boolean function)를 합의 곱(product of sum) 형태로 구하고, 그림 (나)의 전체 구간에 X를 도시하시오.\\n[단계 2] 그림 (가)의 출력 Y와 D-플립플롭의 출력 Q를 그림 (나)의 전체 구간에 각각 도시하시오. (단, Q의 초깃값은 0이다.)",
  "answer":     "[단계1] X = ${xExpression}. (X 파형은 그림 (나) 시뮬레이션 참조)\\n[단계2] Y = ${yExpression}. Q는 X의 상승 에지마다 토글(D=Q'), Y=1이면 비동기 RESET으로 Q=0 (그림 (나) 시뮬레이션 참조).",
  "solution":   "[단계1] (A, B, C) 8개 조합으로 진리표를 만들어 X 값을 채우고 K-map 또는 대수적 변형으로 최소화하면 X = ${xExpression}. 입력 파형의 매 단위 시간마다 (A, B, C) 값을 대입해서 X 파형을 도시.\\n[단계2] Y는 동일 방식으로 ${yExpression}. D-플립플롭은 CLK(=X) 상승 에지에서 D=Q'를 캡쳐하므로 Q가 토글한다. 단, Y=1 구간은 비동기 RESET이 우선 적용돼 Q=0으로 강제. 초기 Q=0에서 시작해 시뮬레이션."
}
...
- JSON 객체 하나만. 코드펜스 금지.`;
  ...
  const enforcedAnswer = `[단계1] X = ${xExpression}.\n[단계2] Y = ${yExpression}, D-FF는 CLK(=X) 상승 에지마다 D=Q'를 캡쳐하므로 Q 토글. Y=1 구간은 비동기 RESET으로 Q=0 강제. (그림 (나) 참조)`;
